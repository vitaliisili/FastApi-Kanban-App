...
def test_get_user_by_id_success(client, test_users):
...
def test_get_user_by_id_error_message(client):
    response = client.get("/api/users/21")
    error = response.json().get("detail")
...
def test_get_user_by_id_not_valid_id(client, id):
    response = client.get(f"/api/users/{id}")
    assert response.status_code == 422


def test_get_user_by_id_not_valid_error_message(client):
    response = client.get(f"/api/users/not_valid_id")
    error = response.json().get("detail")
    assert error == "value is not a valid integer"


def test_update_user_success_update_email(client, test_users):
    data = {
        "id": test_users[0].id,
...
def test_update_user_success_update_first_name(client, test_users):
    data = {
        "id": test_users[0].id,
...
def test_update_user_success_update_last_name(client, test_users):
    data = {
        "id": test_users[0].id,
        "email": test_users[0].email,
        "first_name": test_users[0].first_name,
...
def test_update_user_success_add_admin_role(client, test_users):
    data = {
        "id": test_users[0].id,
        "email": test_users[0].email,
        "first_name": test_users[0].first_name,
...
def test_update_user_not_found_error_message(client):
    data = {
        "id": 99999,
        "email": "user@email.com",
        "first_name": "first_name",
...
def test_update_user_already_exist(client, test_users):
    data = {
        "id": 1,
...
def test_update_user_already_exist_error_message(client, test_users):
    data = {
        "id": 1,
        "email": test_users[1].email,
        "first_name": "first_name",
...
def test_delete_user_success(client, test_users):
...
def test_delete_user_not_found_error_message(client):
    response = client.delete("/api/users/9999")
    error = response.json().get("detail")
...
def test_delete_user_not_found_wrong_id(client, id):
