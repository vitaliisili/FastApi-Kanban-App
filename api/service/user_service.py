...
class UserService:
    ...
    def get_user_by_id(self, id: int, db: Session) -> User:
        user: User = self.user_repository.get_user_by_id(id, db)
        if user is None:
            raise EntityNotFoundException(f"User with id: {id} not found")
        return user

    def update_user(self, user_update: UserUpdate, db: Session) -> User:
        check_user_id = self.user_repository.get_user_by_id(user_update.id, db)
        ...
        if user_update.email != check_user_id.email:
            check_user_email = self.user_repository.get_user_by_email(user_update.email, db)

            if check_user_email is not None:
                raise EntityAlreadyExistsException(f"User with email: {user_update.email} already exists")

        user: User = self.user_repository.update_user(user_update, db)

        return user

    def delete_user(self, id: int, db: Session) -> None:
        check_user_id = self.user_repository.get_user_by_id(id, db)
        if check_user_id is None:
            raise EntityNotFoundException(f"User with id: {id} not found")
